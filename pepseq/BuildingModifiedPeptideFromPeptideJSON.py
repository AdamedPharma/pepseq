...
def add_internal_bond(G, res1_id, atom_name_1, res2_id, atom_name_2):
    Cys1_SG = [
        ...
        if (v.get("AtomName") == atom_name_1
            and v.get("ResID") == str(res1_id))
    ][0]
    Cys2_SG = [
        n
        for n, v in G.nodes(data=True)
        if (v.get("AtomName") == atom_name_2
            and v.get("ResID") == str(res2_id))
    ][0]
...
def get_attachment_points(staple_graph):
    ...
    for dummyAtom in dummyAtoms:
        ...
        attachment_points_on_staple_dict[
            attachment_point_id] = attachment_point
...
def add_staple(
    ...
    staple_graph, attachment_points_on_staple_dict = get_attachment_points(
        staple_graph)

    G_stapled_peptide_union = nx.union(
        peptide_graph, staple_graph, rename=("", prefix))

    for attachment_point_id in attachment_points_on_staple_dict:
        j_peptide_attachment_point = peptide_attachment_points[
            attachment_point_id]

        staple_attachment_point = attachment_points_on_staple_dict[
            attachment_point_id]
        ...
        peptide_attachment_point = find_atom(
            G_stapled_peptide_union, ResID, AtomName)
...
def get_peptide_json_from_sequence(sequence, db_json):
    ...
    N_terminus, C_terminus, sequence_str_wo_termini = find_termini(
        sequence, db_json)
...
def get_molecule_from_sequence(sequence, db_json, N_terminus=None,
                               C_terminus=None):
    canonical_sequence = get_canonical(sequence, db_json)
    symbols_list_w_termini = parse_canonical(canonical_sequence)
    if N_terminus is None:
    ...
    for residue_symbol in residue_symbols:
        smiles_building_blocks_db[residue_symbol] = db_json["smiles"][
            "aa"].get(residue_symbol)["smiles_radical"]
...
def get_molecule_from_json(j, db_json):
    ...
    return get_molecule_from_sequence(
        sequence, db_json, N_terminus, C_terminus)
...
class BuildingModifiedPeptideFromPeptideJSON(object):
    ...
    def execute(self, peptide_json, db_json):
        ...
        for ext_mod in peptide_json["external_modifications"]:
            ...
            attachment_points_on_sequence = ext_mod[
...
def get_smiles_from_peptide_json(peptide_json, db_json):
    mol = BuildingModifiedPeptideFromPeptideJSON().execute(
        peptide_json, db_json)
    smiles = rdkit.Chem.MolToSmiles(mol)
    return smiles
