...
from pepseq.Drawer import draw_symbols, get_kwargs_from_symbols, get_fragment_kwargs, \
   generate_kwargs_for_text_in_ellipse_balls, generate_kwargs_for_ellipse_balls, \
   get_rev_x_and_font_size, get_start_x,  get_N_terminus_params, get_C_terminus_params

 
problematic_symbols = ['CH3', 'Y', 'aMeAla', 'Q', 'G', 'T', 'F', 'T', 'S', 'D', 'Y', 'S', 'K', 'Y', 'L'] + \
    ['D', 'E', 'Cys(R1)', 'A', 'A', 'K', 'D', 'F', 'V', 'Cys(R2)', 'W', 'L', 'L', 'D', 'H', 'H'] + \
    ['P', 'S', 'S', 'G', 'Q', 'P', 'P', 'P', 'S', 'CN', 'C', '123456789', 'NH2']


kwargs_list_as_tuple_mock = ((100, 115, (0.3, 0.3, 0.3), (0.3, 0.3, 0.3), 0, 35),
...
def test_draw_sequence():
    ...
    ['D', 'E', 'Cys(R1)', 'A', 'A', 'K', 'D', 'F', 'V', 'Cys(R2)', 'W', 'L', 'L', 'D', 'H', 'H'] + \
    ['P', 'S', 'S', 'G', 'Q', 'P', 'P', 'P', 'S', 'CN', 'C', '123456789', 'NH2']
    png_string = draw_symbols(copy.deepcopy(symbols), width = 1024, height = 1024)#, out = '../image_file_name_can_err_2.png')    
...
def test_get_kwargs_from_symbols():
    ...
    keys = ['y', 'x', 'rgb_fractions', 'outline_rgb_fractions', 'outline_width', 'radius']
...
def test_generate_kwargs_for_text_in_ellipse_balls():
    ...
    kwargs_for_text_in_ellipse_balls_as_tuple = as_tuple(kwargs_for_text_in_ellipse_balls, keys = ['x', 'y', 'font_size', 'text'])
    assert kwargs_for_text_in_ellipse_balls_as_tuple == fragment_kwargs_txt_as_tuple_mock
...
def test_generate_kwargs_for_text_in_ellipse_balls():
    ...
    kwargs_in_ellipse_balls_as_tuple = as_tuple(kwargs_in_ellipse_balls, keys = [
        'y', 'x', 'rgb_fractions', 'outline_rgb_fractions', 'outline_width', 'radius'])
...
def test_get_fragment_kwargs():
    ...
    fragment_kwargs, fragment_kwargs_txt = get_fragment_kwargs(
        symbols=seq_fragment, y=70, is_start=True, fragment_direction=fragment_direction,
        is_corner=is_corner
    )

    fragment_kwargs_as_tuple = as_tuple(fragment_kwargs, keys=['y', 'x', 'rgb_fractions', 'outline_rgb_fractions', 'outline_width', 'radius'])
    fragment_kwargs_txt_as_tuple = as_tuple(fragment_kwargs_txt, keys=['x', 'y', 'font_size', 'text'])
...
def test_get_C_terminus_params():
    ...
    c_terminus_params = get_C_terminus_params(params=last_params, previous_params=previous_params)
