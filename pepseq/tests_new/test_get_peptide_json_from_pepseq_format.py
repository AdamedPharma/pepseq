import json
import pkgutil
from pepseq.get_peptide_json_from_pepseq_format import get_single_modification_json, get_ext_mod_json, \
    get_smiles_json, get_pep_json

smiles2 = ['[*:1]CNCC[*:2]', '[*:3]CNCC']
symbols_fx = ['Cys(R1)', 'A', 'C', 'D', 'A', 'P', 'E', 'P', 's', 'E', 'Q', 'Cys(R2)', 'A', 'K', 'Cys(R3)']
...
def test_get_single_modification_json():
    ...
    res_fx2 = get_single_modification_json(
                attachment_points_on_sequence_fx, smiles2[1], ketcher=True
            )

    res_fx1 = get_single_modification_json(
                attachment_points_on_sequence_fx, smiles2[0], ketcher=True
            )
...
def test_get_ext_mod_json():
    r = get_ext_mod_json(symbols_fx, smiles2, ketcher=True)
    r_fx = [
        {
            'smiles': '[*:1]CNCC[*:2]',
            'max_attachment_point_id': 2,
...
def test_get_smiles_json():
        ...
        ext_mod = get_smiles_json(symbols_fx, smiles2, ketcher=True)
   ...
   'attachment_points_on_sequence': {3: {'attachment_point_id': '3',
    ...
    peptide_json_fx_2 = {
        ...
        'attachment_points_on_sequence': {'3': {'attachment_point_id': '3',
            ...
            'AtomName': 'SG',
            'ResidueName': 'Cys'}}}]}
    assert peptide_json_fx_2 == get_pep_json(pepseq_format, db_json, ["[*:1]CNCC[*:2]", "[*:3]CNCC"], ketcher=True)
    assert peptide_json_fx == get_pep_json(pepseq_format, db_json, ["[1*]CNCC[2*]", "[3*]CNCC"], ketcher=False)
